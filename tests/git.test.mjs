...
test('Foreign staged files do not enter an automatic commit',async t=>{
 const {a}=await fixture(t);await fs.writeFile(path.join(a.root,'do-not-commit.txt'),'secret');await git(['-C',a.root,'add','do-not-commit.txt']);await a.capture({input:'still saved'});assert.match((await a.status()).gitWarning,/管理外/);assert.equal((await a.list()).length,1);assert.equal(await git(['-C',a.root,'ls-tree','--name-only','HEAD','do-not-commit.txt']),'');
});
