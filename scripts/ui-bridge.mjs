...
const handlers={settings:()=>settings,list:p=>service.list(p),get:p=>service.get(p.id),status:()=>service.status(),capture:p=>service.capture({...p,source:'desktop'}),update:p=>service.update(p.id,p.patch),appendMemo:p=>service.appendMemo(p.id,p.content),updateMemoEntry:p=>service.updateMemoEntry(p.id,p.entryId,p.content),asset:p=>service.asset(p.id),pull:()=>[],reconnect:()=>[],saveSettings:p=>Object.assign(settings,p),openFolder:()=>true,openURL:()=>true};
